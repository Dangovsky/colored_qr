...
def prepare_colors(color_img, size, n_clusters):    
    X = []
    x = size[1] / 2
...
def prepare_img(file_name, n_clusters):
    ...
    for code in codes:        
        ...
        #plt.subplot(121),plt.imshow(color_img),plt.title('persp')
        print(type(rotation_matrix))
        if rotation_matrix is not None:
            color_img = cv2.warpAffine(color_img,rotation_matrix,(color_img.shape[0],color_img.shape[1]))
            plt.subplot(121),plt.imshow(color_img),plt.title('rot')
                        
        plt.subplot(122),plt.imshow(color_img),plt.title('Out')
        prepare_colors(color_img, size_true, n_clusters)
